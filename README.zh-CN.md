...
| 触摸屏 | ⚠️ | 可用 —— Himax HX83121A，需要 `patches/` 里的 gpio174 补丁（[#26](docs/stage4-findings.md)）。**v0.6.2 起手感是实机量出来的**：① 默认预设里的跳点检测阈值实际上是一条 1.0 m/s 的限速线，快滑时驱动一个点都不上报，一次甩动被切成十几次触摸（列表甩不动、手势失灵、"幽灵触摸"）—— 判据改对了（[#114](docs/stage4-findings.md)）；② 坐标 `fuzz` 从 8 改为 0：实测质心抖动不到半个坐标单位，而 fuzz=8 抹掉了四分之一的真实运动数据（[#116](docs/stage4-findings.md)）；③ 按下延迟 25→17 ms；④ 上报触点面积与压力，框架层第一次有了手掌误触抑制的输入。驱动本身修了 6 个缺陷并加了逐级计数器与原始电容帧导出（[#115](docs/stage4-findings.md)）。已知：手掌压屏会碎成多个触点（不影响点击）。|
