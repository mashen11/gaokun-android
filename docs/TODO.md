...
最后更新：2026-09-11（启动失败结案 + 可重建性修好；顺带修掉防漏脚本自己的假阳性）
...
树外源码**（[#79](stage4-findings.md)）。**2026-09-11 已补齐并逐项验收**
（[#80](stage4-findings.md)）：

* `patches/0016`（ashmem，底本 ACK `android15-6.6`）与
  `patches/0017`（xt_quota2，底本 `android16-6.12`）已入库并进 KPATCHES。
  内容取自构建机旧树 `~/gaokun/mainline-linux` —— **当年实测过的那一份**，
  不是重新移植的。6 处 API 漂移已逐条写进补丁头。
* ⚠️★★ 顺带查出 **`kernel-apply-patches.sh` 的指纹判据有假阳性**，
  而且已经让 `patches/0009`（CPU 温控降频）静默漏打 —— **正是 M17 写这个
  脚本要防的那一个补丁**。判据已加固（挑最长的 3 条新增行、要求全部命中）。
* 验收：`.config` 与设备 `/proc/config.gz` **零差异**；内核字符串差集只剩
  构建串；ashmem/quota2 字符串数 **9/10 vs 0/0 vs 9/10**；
  **dtb 与设备现役逐字节相同**；补丁往返零 fuzz。

**⬜ 只剩一件事：上机。** 新内核（`#3`）还没启动过。
⚠️ **上机前要先取得用户同意**（[#79](stage4-findings.md) 第五节的规矩），
并且条目里加 `androidboot.init_fatal_panic=true loglevel=7 printk.devkmsg=on`
—— 上次失败只剩排除法，就是因为没布这一道。

**★ 这条判据进发版收尾清单**（两条都要，缺一不可）：

1. 拿设备的 `/proc/config.gz` 与重建树的 `.config` 做 diff ——
   差出来的每个符号都是一份没入库的源码；
2. 两个内核镜像做**全字符串差集** + **dtb 比 sha256**。
