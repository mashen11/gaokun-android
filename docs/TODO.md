...
最后更新：2026-09-11（启动失败结案：少两份没入库的树外源码；设备复原 + ESP 清理）
...
### A9. ⚠️★★ 内核无法从本仓重建 —— 少两份从未入库的树外源码

**音量补丁内核起不来的原因已查明**（[#79](stage4-findings.md) 第三节，2026-09-11
结案）：照本仓配方从干净的 v7.2-rc2 重建出来的内核，与在跑的 #38 之间，
**功能性差异被穷尽为两项** —— `ashmem` 驱动与 `xt_quota2` 模块。
两个都是本仓 **Stage 3 自己从 ACK 移植**的（[stage2-findings](stage2-findings.md)
8.5 节第 18、20 条），当年都是"Android 起不来 / netd 起不来"级别的硬阻塞，
但**从来没进过 `patches/`**，只活在构建机的内核树里。

⇒ 结论不是"那个补丁有问题"（[#78](stage4-findings.md) 的音量改动站得住），
而是**本仓目前编不出一个能启动的内核**。这是比音量更该先修的事。

**第一步**：开构建机，去 `~/gk3-kernel` 找那两份未跟踪的源码
（`drivers/staging/ashmem/` 之类 + `net/netfilter/xt_quota2.c`，连同它们的
Kconfig/Makefile 改动）。
★ **有时间界标**：ESP 上 08-31 编的相机内核里 `CONFIG_ASHMEM=y` 与
`CONFIG_NETFILTER_XT_MATCH_QUOTA2=y` **都在** ⇒ 源码至少活到 08-31；
09-08 那次是在另起的干净树上编的（构建串 `#2`），如果 `git clean -fd` 过就没了。
**真没了就照第 18 条从 ACK 重新移植**（记得有 2 处 API 漂移要改）。

**第二步**：入库成 `patches/`，并让 `scripts/kernel-apply-patches.sh` 打它们。

**第三步**：把这条判据写进发版收尾清单 ——
**拿设备的 `/proc/config.gz` 与重建树的 `.config` 做 diff，差出来的每个符号
都是一份没入库的源码**。⚠️ 这个坑 M17 已经踩过一次（上游 Venus 补丁集同样
只活在构建机上），当时只补了撞见的那一份，没回头普查，于是原样重演。

⚠️ **再上机测内核时**：条目里加 `androidboot.init_fatal_panic=true
loglevel=7 printk.devkmsg=on`，把 init 的 LOG(FATAL) 转成真 panic 落 pstore。
这次之所以只剩排除法、没有直接证据，就是因为没布这一道。

⚠️ **纪律**：往设备上放没验证过的内核之前，先明说可能要按电源键并取得同意。
"有 oneshot 兜底"只保证盘不坏，不保证不用动手。

> ✅ **设备遗留状态已于 2026-09-11 全部复原**（recovery-ramdisk 搬回、失败内核
> 与其条目删除、临时副本清掉），并顺带清了用途已尽的启动脚手架，
> **ESP 99% → 84%（50 MB 可用）**。剩 4 个条目（`android-a` / `android-b` /
> `int-ubuntu` / `rescue-alpine`），逐条验过引用的文件都在。
> 细节见 [#79](stage4-findings.md) 第四节。
...
* **扬声器音量偏小** ⚠️ **返工中，别当已解决。** 原修法（WSA 数字上限 81→90，
  ...
  ⬜ **上机被 A9 挡着** —— 新内核起不来的原因已查明（缺两份没入库的树外源码，
  与音量改动无关），要先把 A9 做掉才能验这一版。
