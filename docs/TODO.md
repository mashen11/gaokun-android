...
### A9. ⚠️★ 音量补丁内核起不来（**设备当前处于非默认状态，先读这条**）

音量的**结论**是站得住的、改动也已入库（[#78](stage4-findings.md)），
但按那个配方编的内核**在机器上没启动起来**，未查明（[#79](stage4-findings.md)）。

**第一步（按顺序）**：

1. **机器可能还是黑屏的** —— 长按电源键强制关机再开机，会自动回到能用的内核
   （`default = *-android-b.conf`，oneshot 已被 systemd-boot 消耗）。
   现役 `slot_b/Image` 全程没动过（sha `7ec8bf2cec625d5e`）。
2. **复原一处**：`ESP/.../android/slot_a/recovery-ramdisk.img` 被搬到了
   `/data/local/tmp/slot_a-recovery-ramdisk.img`（腾空间用，没删），搬回去。
3. **查启动失败**：`efi_pstore`（`scripts/pstore-ctl.sh`）+ `LoaderEntrySelected`
   —— 后者能区分"加载了新条目但内核挂了"与"根本没走到那个条目"。
4. 查完把 `slot_b_audio/` 连同 `…-android-b-audio.conf` 一起删掉，能腾回 15 MB
   （⚠️ ESP 只剩 5.7 MB / 99%）。

**已知的一条线索**：照本仓配方重建的内核，config 与在跑的 #38 只差
`CONFIG_ASHMEM` 与 `CONFIG_NETFILTER_XT_MATCH_QUOTA2` 两个符号，
而**这两个的 Kconfig 在整棵树里都不存在** ⇒ #38 是从一棵还带着**树外源码**
的树上编的，那部分从未入库。差的可能不止 config 那两行。
⚠️ 这是假说，别当结论。
