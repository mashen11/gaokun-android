...
| # | 事情 | 现在卡在哪 | 下一步（具体） |
|---|---|---|---|
| **T1** | **触摸：幽灵触摸 + 不灵敏** | ✅**主因已查明并当场修掉**：我们自己 game 预设里的 `track_jump_dist2=6400` = 一条 1.0 m/s 限速线，超过就**一个点都不上报** ⇒ 一次甩动被切成 **87 条**轨迹（修后 6 条），沿途多出的那些按下就是幽灵（[#114](stage4-findings.md)） | 预设已清 0、`patches/0038` 已把判据改对。⬜ 要**一次构建**把脚本进镜像；⬜ 另要**一次重启**验 fuzz（`#20` 已编好）与触点面积（`disable_pressure=0`） |
