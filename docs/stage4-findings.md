...
### ⚠️★★★★ 一、最重要的一条：A/B 回落网**不存在**，而全项目一直以为它在
...
⇒ 从 M13 做 A/B 到现在，"另一个槽就是备份"这条纪律（M13 拆 `.bak-prev` 时
写下的理由）**在事实上是空的**。⬜ 要么给 A 槽真的灌一份，要么把文档里所有
"回落到另一个槽"的说法改掉。

★ 方法论：**`bootctl` 说可启动 ≠ 真的能启动。** 判据要用 `lpdump` 看 super
里到底有没有那套分区。
