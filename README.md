...
7. **Camera.** Front camera works in development (see the status table); what is left is packaging the HAL into the ROM, colour calibration for hi846, and the rear S5K3L6 (its power rails are unknown — the device tree wires its 2.8 V analogue supply to an LDO the panel uses at 1.8 V, so board-level evidence is needed before it can be tried).
