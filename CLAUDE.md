...
>
> ### 现在设备上跑的是什么
> ⚠️★★ **用户 2026-09-23 报：槽 `_a` 起不来。** `_a` = 未发布的开发版（戳 `1789737346`，2026-09-18 装，
> `lineage_gaokun3-bp4a-userdebug`，SELinux 第五轮 + 属性改名）。09-18 那次是起来了的（#117 §18），
> 坏在之后某次启动，**原因未查、别先入为主**。⇒ **这一版不能发**；设备现在停在哪个槽未知。
> 回来后的取证顺序见 `docs/TODO.md`「眼前的下一步」。
> ⚠️★★ 槽 `_b` **已不可启动**（`bootctl is-slot-bootable 1` = 0，mapper 里只有 `*_a`，#118 §2）——
> **现在没有回落槽**。重启 / 装内核前把这一点算进风险：`_a` 起不来就只能靠救援系统。
> `default` 由 boot_control 在成功启动后自动指向当前槽（09-18 时是 `_a`；`_a` 起不来之后是否已回落 `_b` 未确认）。
