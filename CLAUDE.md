...
> SELinux 第五轮 + 属性改名）。**它没有发过版。**
> 2026-09-23 实测它**正常启动**（槽 `_a`、`boot_completed=1`、`is-slot-marked-successful 0` = 1，
> pstore 无 09-13 之后的记录）。用户当天说过一句"`_a` 起不来"，但说不清现象、自己也不确定，
> **以实机为准：`_a` 可启动**。若再有人报，先要现象（黑屏 / 反复重启 / 卡在哪一屏）再查。
