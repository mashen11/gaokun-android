...
**当前阶段：Stage 6 M28 — WPA3 结案（不是 WPA3 的问题，是密码对不上，两条独立算法各自证明）+ camss 电源域找到第二个候选修复并已编成内核 `#6` 待测。**★★★★★ WPA3：拿 `wpa_cli` 挂上运行中的 supplicant **绕开框架的 WPA2→WPA3 自动升级**，做出了此前做不出来的 WPA2 对照组。2×2 实测：`SAE`+真密码 与 `SAE`+故意写错的密码**失败签名逐行同形**（`status 15` = AP 校验 SAE Confirm 失败），`WPA-PSK` 两边也同形（关联成功后 `4WAY_HANDSHAKE_TIMEOUT`）。SAE 的 PWE 只由密码+两端 MAC 决定、PSK 走 `PBKDF2(密码,SSID)`，**两条算法只共用密码**。换全新 MAC（排除黑名单）、扫描确认只有那一台同名 AP（排除邻居重名）、`sae_pwe` 0/1/2 全试（排除 H2E 不匹配）。见 [#100](docs/stage4-findings.md)。★★★★ 相机电源域：上游 `bd09d87c55d6` 的提交说明原文就是我们的症状（"**CAMSS_TOP_GDSC** … requires the enablement of the **multimedia NoC**, otherwise the **GDSC will be stuck on 'off'**"），而它需要的 `needs_icc` 基础设施**v7.2-rc2 里已经有、只是没给 sc8280xp 接线**。这次不只是文本像 —— 它能解释此前六条排除**为什么全部落空**，还解释了一条以前没人问过的事实：`ife_0..3` 每次拍照都完整下电上电从不出错，只有 `titan_top` 起不来。⚠️ 仍有一条对不上（开机后第一次上电是成功的），所以**照旧是待验证假说**，内核 `#6` 已放上 ESP、**故意没设 oneshot**。见 [#101](docs/stage4-findings.md)。⬜ 相机日常可用（自研 AIDL HAL + libcamera 软件 ISP），欠账：电源域靠开机钉住当桥、画质未标定。（每次开工时更新这一行）

> ## ★★★ 开工前先读：设备正常，一件事等你在场（2026-09-13）
>
...
>
> **⓪ ⬜ 内核 `#6` 已编好放上 ESP，【故意没设 oneshot】—— 等有人能按电源键。**
> 条目 `…-cam3.conf`，产物 `android/slot_cam3/{Image,gaokun3.dtb}`
> （sha `6facbde1…` / `1273a6a9…`，已与构建机逐字节交叉校验）。
> 带两个补丁：`patches/0021`（camss 电源域**第二个候选修复** —— 给
> `titan_top_gdsc` 加 interconnect 投票，驱动+DT 两半**必须一起上**）与
> `patches/0022`（上游 `86b23609d5e1`，修 [#87](docs/stage4-findings.md) 那个
> unbind 撞名；**与相机判据零交叉**，不破坏单变量测试）。
> **测法**（与 #87 逐字相同才可比）：oneshot 过去 → 开机后跑一次 `camtest`
> （应 12 帧、`titan_top_gdsc` = `on`）→ 等它在 `pm_genpd_summary` 里变
> `off-0` → **再跑一次**。成功 = 修好了；仍 −110 = 假说被否。
> ⚠️ **新内核第一次上机要有人能按电源键。**
> ⚠️ 一旦触发失败，camss 会锁死 `runtime_error`，**只有重启能恢复** ——
> 这也是为什么用户不在场时"先复现再观察"这条路是关着的。
>
> **⓪b ✅ WPA3 结案：不是 WPA3 的问题。** 用户家里那台 AP
